...
- For expressions `e 1 , e 2 , ... , e k` with `k >= 2`, we have `R({e 1 , e 2 , ...}) = R(e 1 ) &cup; R(e 2 ) &cup; ...`

- For expressions `e 1` and `e 2`, we have `R(e 1 + e 2 ) = {a + b for (a, b) in R(e 1 ) &times; R(e 2 )}`, where `+` denotes concatenation, and `&times;` denotes the cartesian product.
